...
static void KeyExpansion(uint8_t *RoundKey, const uint8_t Key[KEY_SIZE])
{
  unsigned i, j, k;
...
void read_data(uint8_t input[16], state_t in_buf)
{
	ap_uint<3> r, c;
...
void write_data(state_t buf, uint8_t output[16])
{
	ap_uint<3> r, c;
...
#pragma HLS ARRAY_PARTITION variable=RoundKey complete dim=1
	...
	if (temp_key != key){
		read_key(key, key_new);
		KeyExpansion(RoundKey, key_new);
		temp_key = key;
	}

	read_data(input.data, in_buf);
	Cipher(in_buf, RoundKey);
	write_data(in_buf, output.data);
	*out_stream = output;
}
